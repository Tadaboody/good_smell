...
class RangeLenSmell(LintSmell):
    def check_for_smell(self) -> bool:
        ...
        return EnumerateFixer(change_node=False).visit(ast.parse(self.source_code))

    def fix_smell(self) -> str:
        """Return a fixed version of the code without the code smell"""
        return astor.to_source(EnumerateFixer(change_node=True).visit(ast.parse(self.source_code)))
